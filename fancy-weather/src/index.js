      ...
      navigator.geolocation.getCurrentPosition(position => {
        ...
        fetch(api)
          ...
          .then(data => {
            ...
            let apparentTemperaturefahren = Math.round((apparentTemperature * 9) / 5 + 32)
            // Translate TO DO
            language.addEventListener('click', function() {
              ...
              feelsLikeTemp.textContent =
                language.value === 'RU'
                  ? `Ощущается как: ${celciusBtn.classList.contains('degree-btn_active') ? Math.round(apparentTemperature) : apparentTemperaturefahren}`
                  : `Feels like: ${celciusBtn.classList.contains('degree-btn_active') ?  Math.round(apparentTemperature) : apparentTemperaturefahren}`
            ...
            function checkDegree(e) {
              if (e.target.id === ferinheightBtn.id) {
                ...
                temperatureDegree.textContent = Math.round(fahren);
                feelsLikeTemp.textContent = `Feels like: ${Math.round(apparentTemperaturefahren)}`
                ...
                feelsLikeTemp.textContent = `Feels like: ${Math.round(temperature)}`
              }
            }
            ...
            feelsLikeTemp.textContent = `Feels like: ${Math.round(apparentTemperature)}`;
